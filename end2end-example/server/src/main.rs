...
async fn boomerang_issuance_m2(data: web::Data<Arc<AppState>>, req_body: String) -> impl Responder {
    ...
    let issuance_m2 = IssuanceS::<Config>::generate_issuance_m2(
        issuance_m1, 
        data.skp.clone(), 
        &mut OsRng);
    ...
    issuance_m2.serialize_compressed(&mut issuance_m2_bytes).unwrap();
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            // configure routes
            .route("/boomerang_issuance_m2", web::post().to(boomerang_issuance_m2))
    })
    .bind(("127.0.0.1", 8080))?
    .run()
    .await
