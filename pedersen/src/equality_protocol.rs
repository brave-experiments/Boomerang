...
impl<P: PedersenConfig> EqualityProof<P> {
    ...
    pub fn add_to_transcript(&self, transcript: &mut Transcript, c1: &sw::Affine<P>, c2: &sw::Affine<P>) {
    ...
    pub fn verify(&self, transcript: &mut Transcript, c1: &sw::Affine<P>, c2: &sw::Affine<P>) -> bool {
        // Make the transcript.
        self.add_to_transcript(transcript, c1, c2);
        self.verify_with_challenge(c1, c2, &transcript.challenge_scalar(b"c")[..])        
    }
        
    pub fn verify_with_challenge(&self, c1: &sw::Affine<P>, c2: &sw::Affine<P>, chal_buf: &[u8]) -> bool {
        let chal = <P as PedersenConfig>::make_challenge_from_buffer(chal_buf);
