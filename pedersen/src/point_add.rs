...
pub trait PointAddProtocol<P: PedersenConfig> {
    ...
    fn create_proof_own_challenge(
        ...
        Self::create_proof(a, b, t, inter, c1, c2, c3, c4, c5, c6, &Self::challenge_scalar(transcript)[..])
    }
    ...
    /// * `ci` - the commitments.
    #[must_use]
    fn verify(&self, transcript: &mut Transcript,
              c1: &sw::Affine<P>,
              c2: &sw::Affine<P>,
              c3: &sw::Affine<P>,
              c4: &sw::Affine<P>,
    ...
    fn verify_with_challenge(&self,                             
                             ...
                             c5: &sw::Affine<P>,
                             c6: &sw::Affine<P>,
                             chal: &<P as CurveConfig>::ScalarField,) -> bool;
    ...
    /// * `transcript` - the transcript object.
    /// * `ci` - the commitments. 
    fn verify_proof_own_challenge(&self, transcript: &mut Transcript,
                                  c1: &sw::Affine<P>,
                                  c2: &sw::Affine<P>,
                                  c3: &sw::Affine<P>,
                                  c4: &sw::Affine<P>,
